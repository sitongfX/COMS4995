...
function init() {
    ...
    window.program = gl.createProgram(); // create the WebGL window.program.  This variable will be used to inject our javascript variables into the window.program.
    gl.attachShader(window.program, vertexShader); // add the shaders to the window.program
    gl.attachShader(window.program, fragmentShader); // ^^
    gl.linkProgram(window.program);     // Tell our WebGL application to use the window.program
    gl.useProgram(window.program); // ^^ yep, but now literally use it.
    
    
    /* 
    
    Alright, so here we're attatching javascript variables to the WebGL code.  First we get the location of the uniform variable inside the window.program. 
    
    We use the gl.getUniformLocation function to do this, and pass thru the window.program variable we created above, as well as the name of the uniform variable in our shader.
    
    */
    locationOfResolution = gl.getUniformLocation(window.program, "u_resolution");
    locationOfTime = gl.getUniformLocation(window.program, "u_time");
    
    
    /*
    
    Then we simply apply our javascript variables to the window.program. 
    ...
    You can also send it over to the window.program as a vector, by using gl.uniform2fv.
...
function render() {
    ...
    window.positionLocation = gl.getAttribLocation(window.program, "a_position"); // do stuff for those vertex's
    gl.enableVertexAttribArray(window.positionLocation);
    gl.vertexAttribPointer(window.positionLocation, 2, gl.FLOAT, false, 0, 0);
